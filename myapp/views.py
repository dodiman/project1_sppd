...
# ====================================================siapa saja=================================
@login_required(login_url='login')
@allowed_users(allowed_roles=['siapasaja'])
def index_siapasaja(request):
	context = {}  
	return render(request, 'myapp/siapasaja/dashboard.html', context)

@login_required(login_url='login')
@allowed_users(allowed_roles=['siapasaja'])
def surat_perintah_siapasaja(request):
	sppd = Sppd.objects.filter(nomor = None)
	suratperintah = Surat_perintah.objects.all().order_by('-created_at')
	jumlah_none = sppd.count()
	...
	return render(request, 'myapp/siapasaja/surat_perintah.html', context)

@login_required(login_url='login')
@allowed_users(allowed_roles=['siapasaja'])
def showSuratePerintahsiapasaja(request, pk):
	instansi = Instansi.objects.first()
	suratperintah = Surat_perintah.objects.get(id=pk)
	suratperintah_pegawai = suratperintah.penanggung_jawab
	koordinator = suratperintah.koordinator
	...
	# return HttpResponse(sppd)
	return render(request, 'myapp/siapasaja/show_surat_perintah.html', context)

@login_required(login_url='login')
@allowed_users(allowed_roles=['siapasaja'])
def sppdsiapasaja(request):
	sppd = Sppd.objects.exclude(nomor = None).order_by('-created_at')
	jumlah_none = Sppd.objects.filter(nomor = None).count()
	...
	return render(request, 'myapp/siapasaja/sppd.html', context)
...
@login_required(login_url='login')
@allowed_users(allowed_roles=['siapasaja'])
def showSppdsiapasaja(request, pk):
	instansi = Instansi.objects.first()	
	sppd = Sppd.objects.get(id=pk)
	...
	return render(request, 'myapp/siapasaja/show_sppd.html', context)

@login_required(login_url='login')
@allowed_users(allowed_roles=['siapasaja'])
def laporansiapasaja(request):
	pengeluaran = Pengeluaran.objects.all()
	# # total_rincian = pengeluaran.rincian.aggregate(Sum('harga'))
	# mylist = list(pengeluaran)
	# kk = ()
	...
	return render(request, 'myapp/siapasaja/laporan.html', context)

@login_required(login_url='login')
@allowed_users(allowed_roles=['siapasaja'])
def pengeluaransiapasaja(request):
	pengeluaran = Pengeluaran.objects.all()

	context = {
		'pengeluaran': pengeluaran
	}
	return render(request, 'myapp/siapasaja/pengeluaran.html', context)

@login_required(login_url='login')
@allowed_users(allowed_roles=['siapasaja'])
def showPengeluaransiapasaja(request, pk):
	instansi = Instansi.objects.first()
	pengeluaran = Pengeluaran.objects.get(id=pk)
	rincian = pengeluaran.rincian.all()
	total_rincian = pengeluaran.rincian.aggregate(Sum('jumlahnya'))
	...
	return render(request, 'myapp/siapasaja/show_pengeluaran.html', context)

@login_required(login_url='login')
@allowed_users(allowed_roles=['siapasaja'])
def showRinciansiapasaja(request, pk):
	pengeluaran = Pengeluaran.objects.get(id=pk)
	rincian = pengeluaran.rincian.all()
	total_rincian = pengeluaran.rincian.aggregate(Sum('jumlahnya'))
	...
	return render(request, 'myapp/siapasaja/show_rincian.html', context)